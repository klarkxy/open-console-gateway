...
test("resolved V3 config equals the previous constants", () => {
  const previous = Object.freeze({
    ...
    driftMessage:
      "Dashboard V3 contract drifted (schema/dashboard-api-v3.schema.json, src/api/generated/dashboard-v3.ts). Run `pnpm run contract:v3:generate`.",
  });
  ...
  assert.equal(
    formatDriftError(spec, [previous.schemaRelativePath, previous.typesRelativePath]),
    previous.driftMessage,
  );
  assert.throws(
