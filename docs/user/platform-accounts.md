...
Refresh is manual. The parent card shows wallet remaining as a **Balance** figure in its body, with lifetime used beside it when known. Each linked Key row can show its own remaining next to its enable switch. Wallet, subscription, and Key limits are separate scopes and are not added together.
