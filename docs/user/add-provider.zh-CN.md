...
5. 补齐账号与 **供应商** 控制面/UI 流程；只在该供应商真实支持时加入目录刷新、启停、验证、错误、冷却、价格和用量。所有面板变更都带 CAS：用户定义供应商的创建走 V4 onboarding commit（`/dashboard/api/v4`）；供应商定义编辑与 V3 上的账号操作走 `/dashboard/api/v3`；Key 轮换、绑定编辑与身份内新增凭据走 V4。
