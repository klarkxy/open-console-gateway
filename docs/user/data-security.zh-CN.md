...
- **Custom API 目的地**：完整的 Custom 推理 Endpoint 由管理员显式信任。允许公网、局域网与回环 HTTP / HTTPS 目的地。元数据、链路本地以及不透明 IPv4 把戏主机会被拒绝。URL 内嵌凭据、query 与 fragment 会被拒绝；携带秘密的请求不会跟随重定向；不会转发 dashboard 或客户端凭据。模型覆盖到另一 Origin 时不会自动继承已保存的 Key。请只填写本节点确实要访问的目的地。
