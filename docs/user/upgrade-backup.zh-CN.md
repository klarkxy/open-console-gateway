...
在执行破坏性 schema 重写（v27、v35、v42、v48、v58）之前，迁移器会先写一个唯一且不覆盖的同级快照——包括 `data.sqlite.pre-v58.<timestamp>.bak`——并附带 SHA-256 sidecar。全新空数据目录直接创建 schema v58，不写这些副本。快照只是回滚点，不能替代完整备份：恢复前先校验 sidecar，并且只能恢复到可打开该 schema 版本的程序上，或用于重试一次从未提交成功的升级。旧版程序无法打开已迁移的数据库。迁移是 fail-closed 的：无法安全迁移的数据会拒绝升级而不是被删除。

### 节点备份载荷

节点备份当前导出 payload V8，以目的地与凭据为权威（密钥与 identity extras 只存在加密信封内），并携带模型解析策略与按模型路由覆盖。V4–V8 备份可导入（V7 会补确定性解析默认值）；payload V1–V3 与 V9 或更新版本会返回明确的不支持版本错误——那不是密码错误，也不是文件损坏。
