...
节点备份当前导出 payload V7，以目的地与凭据为权威（密钥与 identity extras 只存在加密信封内）。用户定义供应商作为目的地 extras。V4–V6 备份仍可导入（V4/V5 沿用旧的冷却仅本机行为）；payload V1–V3 以及 V8 或更新版本会被明确的不支持版本错误拒绝——那不是密码错误，也不是文件损坏。
