...
新发现模型会按官方文档已知的协议或连接已配置的路由保存为启用。已有模型若已确认支持但被关闭，或由你明确关闭，仍保持关闭。没有协议证据的模型会等待官方资料；之后有证据时，刷新可以补充并启用该协议。已有映射、首选协议、路由覆盖、Key 授权和探测观察都会保留；刷新可以补充新近文档化的协议声明。失败、空结果或过时结果保留原目录；部分响应会明确标为部分结果。
