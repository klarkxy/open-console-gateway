...
Step Plan (CN) has no official usage API in Open Console Gateway. The account card uses a **local estimate** plus manual calibration. Pick Mini / Plus / Pro / Max (400M / 1600M / 8000M / 40000M credits, 1M credits = 1 CNY). Rates come from the server preset. Setup asks for the current remaining balance and the next reset (default: next calendar month 00:00 China time, UTC+8) so the first observation is complete. No console cookie or login is required.
