...
创建预设供应商时，[渠道预设](provider-presets.zh-CN.md) 统一展示在一个列表中，移除 Plan/API 分组。Custom API 固定首位，其余按名称排列；厂商的服务版本仍可在详情中选择。这些模板与默认展示的已配置连接分开。固定预设显示准确连接摘要，并填入可修改的默认对话模型；Azure、Bedrock 仍需资源或地区地址，以及部署或模型信息。从 **供应商 → 添加供应商** 或 **账号 → 新增账号**（同一套选择器）**保存草稿** 会把连接留在列表并标为 **草稿**（不参与路由）。**完成设置** 需要模型；keyed 鉴权还需要 Key，或在继续设置时保留已保存 Key。两个入口使用同一次 onboarding commit。模板变化不会改写已保存连接。
