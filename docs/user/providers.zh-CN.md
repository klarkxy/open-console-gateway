...
左侧列表展示已保存的用户定义供应商（预设或自定义），不论是否已有账号，以及至少已有一个账号的内置供应商，按 Plan/API 分组。未使用的内置模板不出现在列表，只留在 **添加供应商** 目录中。没有账号的需 Key 供应商显示为 **待补充凭据**：定义已保存，但没有 Key，不参与路由，也不会自动测试。详情里的 **添加 Key** 打开与 **账号** 页相同的凭据编辑器，并预填该供应商。列表底部的 **添加供应商** 在主区打开预设浏览，待添加模板与已经配置的供应商分别展示；已有预设书签会打开同一个嵌入式创建表单。该表单在 keyed 鉴权下 Key 为可选：填写则同一次写入创建首个账号，留空则只保存定义。已保存连接在来源预设明确时保留品牌标识，但不会把修改过的地址认证为官方地址。模型目录有独立的模型搜索和启用状态筛选，供应商列表搜索不代替模型搜索。窄屏下仍可查看模型映射的公开名称和上游名称。
...
创建预设供应商时，可按 Plan/API、厂商和地区变体浏览[渠道预设](provider-presets.zh-CN.md)。这些模板与默认展示的已配置连接分开。固定预设显示准确连接摘要，并填入可修改的默认对话模型；Azure、Bedrock 仍需资源或地区地址，以及部署或模型信息。在 **供应商** 页保存可以省略 Key，定义会以 **待补充凭据** 留在列表；填写 Key 则同一次写入创建首个账号。在 **账号 → 新增账号** 保存仍必须填写 Key，并同时创建供应商与首个账号。模板变化不会改写已保存连接。
