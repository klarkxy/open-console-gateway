...
- Command Code GOAT 是已上线的固定官方源路由。其公开 `/models` 目录在 **供应商** 页显式刷新，账号卡点 **刷新额度** 时也会刷新；GOAT 预设与新发现的模型在已知受支持协议时默认开启，但 GOAT 首次快照仅默认开启套餐包含的模型，该次快照中的其余行保持关闭，可手动开启。GOAT 目录刷新更新模型目录；Key 鉴权从推理 401/403 观察。已验证价格快照通过每个已定价模型可保存的手动倍率估算新请求成本。账号卡可显式 **刷新额度**，从 Command Code 官方 CLI 使用的第一方 `/alpha/billing/credits` 端点读取本地 `$14 / $35 / $70` 三个窗口，再继续累计 OCG 内已定价日志。公开 Provider API 文档未列出该端点，GOAT 不做自动同步。官方刷新之后可以用手工校准三个窗口。Custom API 已在 [账号](accounts.zh-CN.md) 的受信管理员边界下上线路由；可按账号配置本地积分估算，未配置时不计价，也没有通用的官方用量路径，其目录、协议与价格控制作为隔离的 `CustomEndpoint` 范围呈现在 **供应商** 页。
- 账号计费分三种模型：定时额度窗口、现金（官网 API 钱包）、点数；来源独立为官网、本地估算或不可用。没有官网用量 API 的方案使用本地估算，并手工校准。点数费用按实际归一化 Token × 所选每百万费率 × 点数/货币换算，不是订阅标价。只有经过 Open Console Gateway 的流量会进入估算；OCG 之外的用量需要手工校准。未知定价就是未知，不是免费。不完整观测保持不完整，不会填成 0。待结算完成后再校准；仍有待结算请求时拒绝校准。月度发放、充值和过期是分开的桶；剩余不含已过期充值，也不会编造一个共用重置时间。估算本身不停用 Key、不改路由。官网 API 现金余额不变：剩余是官网钱包，本月和历史仍是本地估算。
- Ollama Cloud 每月 USD Credits 用量是本地已定价日志的软估算。实际已用可以超过 Pro `$60` / Max `$300` / Team `$1000` 上限；面板把进度条钳在 100% 并显示超出量。进度条满了绝不写冷却、不停用账号、不改路由。新建 Ollama 账号必须明确选择 Pro/Max/Team 并填写购买日期。无计费行的既有账号仍可路由且无进度条。真实上游 `429` 走通用冷却/回退。
- Zen Free 路由使用账号卡的启用开关和列表位置。
- 未知模型名在所有受支持的客户端格式上返回 `400`。客户端应发送带鉴权的 `GET /v1/models` 公布的、当前有有效启用协议的别名或合格 Custom ID。受保护的 `GET /dashboard/api/v4/application-models` 是 Go 别名 ∩ 当前价格快照，不是那份完整客户端列表。
