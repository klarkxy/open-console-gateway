...
供应商选项严格按 V3 Provider Catalog 返回的顺序投影，选择器、筛选、弹窗与缓存都使用 `provider_id`。成功返回空目录就保持为空；目录加载失败时只保留 OpenCode Go 创建表单，既有 Zen Free 单例仍可展示，其余内置、Custom 与用户定义入口全部失败关闭。名称、Plan/API 分组、创建状态与表单字段都来自目录行。密封供应商的第一张就绪账号保存后，面板先读取该供应商现有 Contract 的目录刷新能力，再决定是否刷新；追加第二把 Key 不重复刷新。能力查询或刷新失败不会回滚已经保存的账号，可到 **供应商 → 刷新模型目录** 重试。
...
账号使用与创建时相同的表单编辑。就绪且可路由的卡片只显示已启用、已禁用、冷却或不可用。卡片上的关系来自身份投影：声明关系不是已验证钱包；动态或 Custom 的日期保持未知，除非 V3 里本来就有真实的购买日期。
...
当前 V7 payload 会迁移目的地、凭据、可用普通账号及其稳定 ID、ready 账号 Key、Custom Endpoint/公开模型 → 上游 ID 映射与验证状态、
用户定义供应商（只有 `providerId`）、
主接入 Key 和所有有效子 Key、可迁移的路由/代理设置、Zen Free 启用状态与目录，Provider
目录、证据和协议覆盖，以及一份显式的身份 / 凭据 / 绑定 / 额度池快照。
