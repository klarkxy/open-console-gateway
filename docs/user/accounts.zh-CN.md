...
**已启用** 表示这张卡可以进入路由。新建就绪 Key 账号（含 Custom API 与用户定义供应商）默认启用。测试连接不会改变开关。已经开启或关闭的账号保持原状态。测试结果保留在测试弹窗中。用户定义供应商没有建模订阅周期，包括从 Plan 预设创建的供应商：其账号不显示推算的购买日期、到期倒计时或到期提醒。既有存储中的购买日期锚点保留兼容，但不作为已经确认的计费事实展示。

账号使用与创建时相同的表单编辑。就绪且可路由的卡片只显示已启用、已禁用、冷却或不可用。卡片上的关系来自身份投影：声明关系不是已验证钱包；动态或 Custom 的日期保持未知，除非本来就存有真实的购买日期。

就绪的 Key 卡片可从溢出菜单 **轮换 Key**、**添加 Key** 和 **编辑绑定**。轮换只替换本机此后请求使用的 Key（凭据 id 不变，版本号递增），是本地替换：不会撤销供应商侧的原凭据，仍由你自行管理。**添加 Key** 会在这条连接上再创建一把推理 Key。额度默认独立；只有你明确选择与同一身份下某把已有推理 Key 共享时才会共享，不会因为使用同一连接或身份而自动共享。保存后，卡片按已存储的额度池显示实际共享关系（能确定时会写出共享的那把 Key）。第三把 Key 若有自己的池或没有池，仍是独立额度。包括迁移后 Custom API 记录在内的可配置 HTTP 连接都支持多把 Key。Zen Free、CPA、无鉴权和观察者凭据仍是单例或由外部管理，不提供添加 Key。若创建没有得到确定结果，表单会锁定已提交的内容和操作：用相同内容重试，或取消；不要改内容后再提交（可能产生重复 Key）。

编辑绑定可改该凭据绑定的启用状态、模型范围（全部模型，或仅你列出的准确名称），以及——仅在你改动时——目标同意：允许此 Key 发往哪些已配置端点（协议和 URL）。已保存的目标是当时的事实；供应商地址日后更改时，仍显示已保存的 Origin，该端点保持未勾选，除非你明确允许新目标。只改范围或启用时不会改目标。两个目标列表都清空即撤销。没有 URL 的官方锁定端点保持官方目标，不会编造 Origin 字符串。绑定禁用显示在该卡片上，不会拨动账号启用开关。Zen Free、CPA、无鉴权和观察者凭据不提供轮换或绑定。一个身份可以持有多把 Key；每张卡片只用 `legacy` 账号 id 与自己匹配的那份凭据。
...
| Custom API | `custom` | 是 | 迁移后可配置 HTTP 连接的兼容身份；一条连接拥有 API 地址、鉴权、协议和公开名称 → 上游 ID 映射，可挂多张 Key 卡；既有记录保持独立，并保留仅公开名称解析；费用 unpriced/unknown，不扣额度 |
...
当前 V8 payload 以目的地与凭据为权威（ready Key、平台与 CPA observer 管理凭据，以及 identity / grant / cooldown extras 只存在加密信封内），
...
浏览器 Profile/Cookie、第三方登录密码、邀请码、日志和用量历史不会迁移。V8 会携带来源端冷却截止时间，且不缩短目标端更晚的截止时间；V4/V5 的冷却行为只作用于本机。同 ID
...
会直接跳过。导入接受 payload V4、V5、V6、V7 与 V8。V4/V5 包会为每个账号重建一个身份、一份凭据、一条 All 范围绑定和该身份的额度池。导入 V1–V3 或 V9 及更新备份时会提示版本不受支持。若 V4/V5 文件已经带上 V6 身份字段，或 V6 文件已经带上 V7 目的地字段，导入会拒绝而不是静默丢弃。
...
- **优先级顺序**：账号列表按目的地（供应商、Custom API 地址或平台站点）分组显示 Key，所有组共用一个全局路由顺序。只有一把 Key 的目的地显示为一张卡；有多把 Key 的目的地显示目的地头部和每把 Key 一行。卡片左侧的拖动手柄移动整个组，鼠标、触屏和触控笔都可以使用；聚焦手柄后也可以按上、下方向键移动。组内的 Key 用行菜单里的 **上移** / **下移** 调整顺序，不会离开所在组。排序保存在当前节点的
