...
**已启用** 表示这张卡可以进入路由。新建 Key 账号（含 Custom API 与用户定义供应商）默认关闭。测试连接不会打开开关；验通后请自行启用。已经开启的账号保持原状态。测试结果保留在测试弹窗中。用户定义供应商没有建模订阅周期，包括从 Plan 预设创建的供应商：其账号不显示推算的购买日期、到期倒计时或到期提醒。既有存储中的购买日期锚点保留兼容，但不作为已经确认的计费事实展示。
...
| Custom API | `custom` | 是 | 受信管理员目的地；每张账号卡保存一个 API 地址、一个账号级上游协议，以及多条公开名称 → 上游 ID 映射；常见基址会自动补全；新建账号默认关闭；合格公开名称会出现在 `/v1/models`；费用 unpriced/unknown，不扣额度 |
...
GOAT 目录刷新更新模型目录；Key 鉴权从推理 401/403 观察。enabled、ready 且 Key 非空的账号可路由供应商矩阵中已启用的模型。新建且可路由的 Custom API 账号默认关闭；编辑 Endpoint、能力、Key 或协议仍保持当前启用状态。禁用草稿仍可保存。
