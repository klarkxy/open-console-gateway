...
账号仍通过同一套表单编辑。就绪且可路由的卡片只显示已启用、已禁用、冷却或不可用。卡片上的关系来自身份投影：声明关系不是已验证钱包；动态或 Custom 的日期保持未知，除非 V3 里本来就有真实的购买日期。
...
当前 V6 payload 会迁移可用普通账号及其稳定 ID、ready 账号 Key、Custom Endpoint/公开模型 → 上游 ID 映射与验证状态、
...
浏览器 Profile/Cookie、第三方登录密码、邀请码、日志和用量历史不会迁移。V6 会携带来源端冷却截止时间，且不缩短目标端更晚的截止时间；V4/V5 保留旧版冷却只属于本机的行为。同 ID
...
会直接跳过。导入接受 payload V4、V5 与 V6。V4/V5 包会为每个账号重建一个身份、一份凭据、一条 All 范围绑定和该身份的额度池。导入 V1–V3 或未来的 V7 备份时会提示版本不受支持。若 V4/V5 文件已经带上 V6 身份字段，导入会拒绝而不是静默丢弃。
...
给现有供应商追加 Key 时，选择已有连接，不会再创建另一份供应商配置。新服务入口包含未使用的内置模板、Plan/API 预设、Custom API 与平台类型。搜索匹配厂商、变体、预设名称和端点主机；选中搜索结果后，清空搜索仍保留准确变体。Zen Free 是后端管理的单例，只在账号列表启停；OpenCode Go 在宿主支持时仍提供可选的托管注册入口。
...
- **刷新额度（已完成的 Key / 托管账号）**：官方用量（`/zen/go/v1/usage`）只是周期性校准基线；本节点转发日志成本仍是实时估算。活跃的
  ...
  按统一的范围限制策略处理，并额外在约 1–2 分钟后调度一次官方对账；官方失败或
...
- **GOAT 推理限制**：已识别的 5 小时、周或月套餐重置写入精确匹配的冷却。`Retry-After` 独立约束重试，不替代额度重置。没有重试约束的未知或临时 429 只影响本次请求；精确匹配的余额不足 400 则进入已声明额度池内同模型等待和前台单飞重探，不停用 Key，也不写鉴权错误。详见[路由与恢复](routing.zh-CN.md)。
...
- **解除冷却**：现有可见的冷却操作会让进度条恢复本地估算值；其 CAS 后端操作还会清除关联进程内等待，并隔离晚到响应。账号级解除不重置匿名 Free 限制。本次没有为仅本地等待的资源新增标签或按钮，可查看请求诊断中的 `resource_wait`。重启清空本地等待，但保留持久化额度截止时刻；历史通用冷却记录不会迁移。
