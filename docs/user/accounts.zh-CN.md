...
**新增账号** 先区分已有连接与新服务。选择已有供应商，会沿用其已保存的地址、协议和模型，只添加另一把 Key，也包括在 **供应商** 页保存、尚未填写 Key 的用户定义供应商；选择新服务，再浏览 Plan/API 预设或添加平台站点。保存预设会同时创建供应商和首个账号。填写 Key 前始终可查看连接摘要，地区和套餐变体使用紧凑选择器。Key 由账号服务保存；你可以在本页添加，也可以在供应商详情里用 **添加 Key**。
