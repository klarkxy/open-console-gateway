...
Rotate, binding edits, and second-credential writes are V4 CAS paths on credentials. New node exports use portable payload V7 (envelope remains v1). V7 carries destinations and credentials as the authority, including plaintext secrets and identity / grant / cooldown extras inside that envelope, plus quota pools and node state. Latest export does not emit a parallel accounts / platform / identities graph. V4–V6 packages remain importable through an old-graph decoder that maps into the same new-model import object. A V7 package that still carries leftover old fields must match dest/cred or is rejected. A V4/V5 package that already contains V6 identity fields, or a V6 package that already contains V7 destination fields, is rejected rather than silently stripped. Payload V8 and newer are rejected with an explicit unsupported-version error. Schema v52 later dropped `accounts`; v53–v57 dropped the leftover Custom, platform, CPA, Provider, and identity satellite tables. Destinations + credentials are the store.
