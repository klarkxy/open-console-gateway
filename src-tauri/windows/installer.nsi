...
; Product lifecycle: install, in-place update, uninstall, reinstall.
; Do not restore Tauri's nested "uninstall before installing" page.
Unicode true
...
!define LEGACYPRODUCTNAME "OCG Manager"
...
; Existing copies are replaced in place. Uninstall is only offered from
; Windows Installed apps, never from this installer.
; 4. Choose install directory page (skipped for updates and live copies)
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassiveOrExisting
...
Function .onInit
  ...
  ${If} $UpdateMode = 1
    ; Signed updates ignore /D= so they cannot create a second copy.
...
Section Install
  ...
  ; Replace hashed frontend assets instead of uninstalling the previous copy.
...
FunctionEnd
Function SkipIfPassiveOrExisting
  ${If} $PassiveMode = 1
  ${OrIf} $UpdateMode = 1
