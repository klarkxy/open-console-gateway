...
#[tokio::test]
async fn waiting_skips_network_and_complete_priority_trial_restores_key() {
...
async fn malformed_success_does_not_release_a_probe() {
    ...
    clock.store(1802, Ordering::SeqCst);
    succeeds(&h).await;
    succeeds(&h).await;
...
async fn credential_rotation_and_explicit_reset_allow_retry_without_waiting() {
    ...
    let (status, result) = dashboard_json(
        ...
        "v4",
        &format!(
            "/credentials/{}/quota-retry",
            identity_refs_for(&h.state, &ids[0]).credential_id
        ),
        Some(&body),
    )
    .await;
...
#[tokio::test]
async fn recognized_per_key_quota_does_not_restrict_a_declared_pool_sibling() {
    let p = PreparedFallback::routing(
        &[
            ("a", &[reply(400, CREDITS)]),
            ("sibling", &[ok()]),
    ...
    assert!(
        !h.logs()
            .iter()
            .any(|r| r.account_id == sibling && r.error_stage.as_deref() == Some("resource_wait"))
    );
...
#[tokio::test]
async fn confirmed_quota_retry_after_waits_for_this_key_without_a_second_probe() {
    use axum::{http::HeaderMap, response::IntoResponse};
    let calls = Arc::new(std::sync::Mutex::new(Vec::<String>::new()));
    let recorded = calls.clone();
    let app = axum::Router::new().fallback(axum::routing::post(move |headers: HeaderMap| {
        ...
        async move {
            ...
            let fail = {
                ...
                let fail = key == "a" && !calls.iter().any(|value| value == "a");
                calls.push(key);
                fail
            };
            if fail {
    ...
    h.push_stop(stop);
    succeeds(&h).await;
    let credential = identity_refs_for(&h.state, &ids[0]).credential_id;
    ...
    assert!(
        row["quotaRecovery"]["resetsAt"].is_null(),
        "Retry-After is not a quota reset"
    );
    clock.store(901, Ordering::SeqCst);
    succeeds(&h).await;
    ...
    assert!(
        h.logs()
            .iter()
            .any(|row| row.account_id == ids[0]
                && row.error_stage.as_deref() == Some("resource_wait"))
    );
    clock.store(3601, Ordering::SeqCst);
    succeeds(&h).await;
    succeeds(&h).await;
    assert_eq!(*calls.lock().unwrap(), ["a", "b", "b", "a", "a"]);
    let (_, view) = v4_get(h.port, "/credentials").await;
    let row = view["credentials"]
        .as_array()
        .unwrap()
        .iter()
        .find(|row| row["id"] == credential)
        .unwrap();
    assert!(row["quotaRecovery"].is_null());
    drop(h);
    server.await.unwrap();
}
