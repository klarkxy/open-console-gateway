...
use crate::dashboard_v3::{ControlRevision, MutationExpectation, V3ApiError, check_expectation, parse_mutation_json};
...
pub(super) async fn get(State(state): State<CoreState>, Path(id): Path<String>) -> Result<Json<DestinationModelMetadata>, V3ApiError> {
    let _settings = state.settings_update.lock();
    payload(&state, &id).map(Json)
}

pub(super) async fn put(State(state): State<CoreState>, Path(id): Path<String>, body: Bytes) -> Result<Json<DestinationModelMetadata>, V3ApiError> {
    ...
    if json.get("metadata").is_none() {
        return Err(V3ApiError::invalid_request_at(&state, "metadata is required (use null to reset)"));
    }
    if let Some(metadata) = &input.metadata {
        metadata.validate().map_err(|message| V3ApiError::invalid_request_at(&state, message))?;
    }
    let _settings = state.settings_update.lock();
    check_expectation(&state, &input.expectation)?;
    let snapshot = crate::routing_snapshot::RoutingSnapshot::load(&state.db.lock()).map_err(V3ApiError::internal)?;
    let destination = snapshot.projection.destinations.iter().find(|d| d.id == id)
        .ok_or_else(|| V3ApiError::not_found_at(&state, "destination not found"))?;
    let model = destination.catalog.iter().find(|m| m.public_model == input.public_model)
        .ok_or_else(|| V3ApiError::not_found_at(&state, "exact public model not found"))?;
    state.commit_configuration_update(|db| model_metadata::declare(db, destination, model, input.metadata))
...
fn payload(state: &CoreState, id: &str) -> Result<DestinationModelMetadata, V3ApiError> {
    ...
    let models = destination.catalog.iter().map(|model| {
        let (metadata, source) = model_metadata::effective(&records, destination, model);
        DestinationModelMetadataEntry { public_model: model.public_model.clone(), upstream_model: model.upstream_model.clone(), metadata, source: source.to_string() }
    }).collect();
    Ok(DestinationModelMetadata { revision: ControlRevision::from_state(state), destination_id: id.to_string(), models })
}
