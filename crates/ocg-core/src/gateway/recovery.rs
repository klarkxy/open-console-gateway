    ...
    pub(crate) fn acquire(
        ...
        inner.slots.retain(|key, slot| {
            matches!(
                ...
                || slot.owners != resources.members
                || slot.owner_generation == resources.quota_generation()
        });
        let new_count = keys
            .iter()
        ...
        let claims = keys
            ...
            .map(|key| {
                ...
                slot.owner_generation = resources.quota_generation();
                for owner in &resources.members {
                    if !slot.owners.contains(owner) {
                        slot.owners.push(owner.clone());
                    }
                }
                slot.active += 1;
                let probe = resources.enforces(&key) && slot.restricted();
                if probe {
...
impl RecoveryPermit {
    ...
    pub(crate) fn observe_failure(
        ...
        let key = self.resources.for_facts(facts);
