...
//! control-plane mutation, including `/auth/register`, `/auth/login`,
//! `/auth/logout`, and `POST /accounts/{id}/usage/refresh`. Pricing mutations
//! also require `expectedPricingRevision`. Operational diagnostics such as
//! `POST /settings/test-proxy` are not mutations and neither require nor accept
//! CAS tokens. Plaintext keys must not appear on
...
mod tests {
    ...
    fn catalog_type_names_append_pricing_dtos_after_the_provider_prefix() {
        ...
        let usage_refresh_end = proxy_end + USAGE_REFRESH_CATALOG_TYPES.len();
        assert_eq!(
            &CATALOG_TYPE_NAMES[proxy_end..usage_refresh_end],
