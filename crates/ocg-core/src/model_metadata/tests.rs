...
fn metadata(context: u64, output: u64, inputs: &[&str], efforts: &[(&str, &str)]) -> ModelMetadata {
    ModelMetadata {
        context_window: Some(context), max_output_tokens: Some(output),
        input_modalities: Some(inputs.iter().map(|s| s.to_string()).collect()),
        reasoning: Some(true),
        reasoning_efforts: Some(efforts.iter().map(|(k, v)| (k.to_string(), v.to_string())).collect()),
        ..Default::default()
    }
}
...
fn unknown_is_not_a_fabricated_capacity_or_effort_list() {
    let rows = parse_catalog(br#"{"data":[{"id":"gpt-private"},{"id":"thinking","reasoning":true}]}"#);
...
fn fallback_alias_uses_minimum_limits_and_capability_intersection() {
    let a = metadata(262144, 32768, &["text", "image"], &[("low", "low"), ("high", "high"), ("xhigh", "max")]);
    let b = metadata(131072, 16384, &["text"], &[("low", "low"), ("high", "default")]);
    ...
    assert_eq!(result.reasoning_efforts, Some(BTreeMap::from([("low".into(), "low".into())])));
}

#[test]
fn unknown_fallback_blocks_positive_claims() {
    let result = common(&[metadata(262144, 32768, &["text", "image"], &[("high", "high")]), ModelMetadata::default()]);
...
fn invalid_metadata_is_rejected() {
    for value in [json!({"contextWindow":0}), json!({"contextWindow":100,"maxOutputTokens":200}),
        json!({"reasoningEfforts":{"ultra":"ultra"}}), json!({"reasoning":false,"reasoningEfforts":{"high":"high"}}),
        json!({"inputModalities":["text","text"]}), json!({"reasoningEfforts":{"high":"bad\nvalue"}}),
...
fn duplicate_rows_only_keep_common_guarantees() {
    let rows = parse_catalog(br#"{"data":[{"id":"a","contextWindow":8000},{"id":"a","contextWindow":4000}]}"#);
